...
`src/faces/face_database.py::save_faces()` marque ensuite `ignored=1` (visage conservé en base, masqué de l'UI/clustering, **récupérable**) selon un seuil proportionnel à la résolution de la photo (`_AUTO_IGNORE_MIN_SIDE_RATIO` = 3 % du plus petit côté, relevé à 20 % si un visage "premier plan" est présent) et `_AUTO_IGNORE_MIN_SCORE` (0.65). C'est le seul étage qui doit décider si un petit visage est bruit ou non — **Visages › Ré-évaluer les visages ignorés par taille…** ne peut recalculer que ce flag.
